...
|-------|---------|------------------|-------|
| FHIR-54046 | SubscriptionTopic.trigger.event should be 0..* | Anchor | ADT use case - multiple events share same config |
| FHIR-54049 | SubscriptionTopic cannot indicate required filters/payload | Anchor | Servers need to communicate mandatory constraints |
| FHIR-54070 | Subscription.filterBy.comparator seems redundant | Satellite | Comparator can be in value; OR logic limitation |

### Async Patterns
| Issue | Summary | Anchor/Satellite | Notes |
|-------|---------|------------------|-------|
| FHIR-50598 | _outputFormat handling depends on server support | Anchor | Silent breaking change when servers add capabilities |

## Suggested Discussion Order
1. **FHIR-54046** - Event cardinality (straightforward enhancement)
2. **FHIR-54049** - Required filters/payload (important for IGs)
3. **FHIR-54070** - Comparator redundancy (may be quick)
4. **FHIR-50598** - Async _outputFormat (may need broader async discussion)

## Cross-Theme Dependencies
- **Bundle Quarter**: Async patterns relate to Bundle.response (FHIR-54003)
